...
module decode(
    ...
    always @(*) begin
        ...
        if (inst[27:4] == `BX_INST) begin
            ...
            // No destination register here
            wr_en_a = 1'b0;
            wr_en_b = inst[24];     // Link bit - ALU outputs next_pc from out2 on an add operation
            wr_addr_a = 4'b0;
            ...
            op2_shift_func = `ROR;
            op2_imm_shift_by = {{4{1'b0}}, inst[11:8]};
            op2_is_imm_shift = 1'b1;
